...
def sparse_continents
  ...
  # countries belonging to continent where
  #
  #
  # Hint: Sometimes rewording the problem can help you see the solution.
  execute(<<-SQL)
  SELECT
    smaller_countries.name,
    smaller_countries.continent,
    smaller_countries.population
  FROM
    countries AS smaller_countries
  WHERE
    smaller_countries.continent NOT IN
    (
      SELECT
        larger_countries.continent
      FROM
        countries AS larger_countries
      WHERE
        larger_countries.population > 25000000
    )
  SQL
end
